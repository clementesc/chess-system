...
public class ChessMatch {
    ...
    private void validateSourcePosition(Position position){
        if (!board.thereIsAPiece(position)){
            throw new ChessException("Nao ha peca na posicao origem");
        }
        if (currentPlayer != ((ChessPiece) board.piece(position)).getColor()){
            throw new ChessException("A peca escolhida nao e sua.");
        }
        if (!board.piece(position).isThereAnyPossibleMove()){
            throw new ChessException("Nao existem movimentos possiveis para peca escolhida.");
        }
    }
    ...
    private Piece makeMove(Position source, Position target){
        ...
        // "Roque pequeno" ou "Roque do lado do Rei"
        ...
        // "Roque grande" ou "Roque do lado da Rainha"
        if (p instanceof King && target.getColumn() == source.getColumn() - 2) {
    ...
    private boolean testCheckMate(Color color){
        ...
        // Todas as peças da cor
        List<Piece> list = pieceOnTheBoard.stream().filter(x -> ((ChessPiece)x).getColor() == color).collect(Collectors.toList());
        for(Piece p : list){
            boolean[][] mat = p.possibleMoves();
            for (int i=0; i<board.getRows(); i++){
                for (int j=0; j<board.getColumns(); j++){
                    // é um movimento posssível
                    if (mat[i][j]) {
